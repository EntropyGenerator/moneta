...
DIR=chroot
PREINSTALL_PKGS=openssh-server,curl,tar,gcc,g++,cmake,libc6-dev,time,strace,sudo,less,psmisc,selinux-utils,policycoreutils,checkpolicy,selinux-policy-default,firmware-atheros,debian-ports-archive-keyring

# If ADD_PACKAGE is not defined as an external environment variable, use our default packages
if [ -z ${ADD_PACKAGE+x} ]; then
    ADD_PACKAGE="make,git,vim,tmux,usbutils,tcpdump,lshw,wget,pciutils,software-properties-common,libxml2,autoconf,automake,gawk"
fi
...
DEBOOTSTRAP_PARAMS="--arch=$DEBARCH --include=$PREINSTALL_PKGS --components=main,contrib,non-free $DIST $DIR"
if [ $FOREIGN = "true" ]; then
...
echo "moneta" | sudo tee $DIR/etc/hostname
ssh-keygen -f $DIST.id_rsa -t rsa -N ''
sudo mkdir -p $DIR/root/.ssh/
sudo mkdir -p $DIR/lib/modules/
cat $DIST.id_rsa.pub | sudo tee $DIR/root/.ssh/authorized_keys
...
sudo mkfs.ext4 -F $DIST.img
sudo mkdir -p /mnt/$DIR
sudo mount -o loop $DIST.img /mnt/$DIR
...
qemu-img convert -f raw -O qcow2 $MONBUILD/$DIST.img $MONBUILD/$DIST.qcow2